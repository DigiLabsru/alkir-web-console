...
from .del_sessions import del_sessions
from .get_sessions_per_base import get_sessions_per_base


def show_sessions_per_base(connect_info, main_container, base_name: str):
    data = get_sessions_per_base(connect_info=connect_info, base_name=base_name)
...
async def drop_session(table, connect_info, header, del_button):
    ...
    await run.io_bound(del_sessions, session_to_kill=session_to_kill, connect_info=connect_info)
    update_table_data(table=table, header=header, connect_info=connect_info)
    del_button.enable()


async def update_table_data(table, header, connect_info):
    # new_data = get_sessions_per_base(connect_info=connect_info)
    new_data = await run.io_bound(get_sessions_per_base, connect_info=connect_info)
