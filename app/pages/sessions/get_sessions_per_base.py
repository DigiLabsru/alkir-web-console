...
from ...common.variables.app_translate import app_translate
from .get_sessions_per_base_table import columns


def get_sessions_per_base(connect_info, base_name: str):
    try:
        rac = RasInterface(req=connect_info)
        session_all = rac.java_get_sessions()
        infobases_list = rac.java_get_infobases_short()
        rac.close()
        base_id = [_.getInfoBaseId().toString() for _ in infobases_list if _.getName() == base_name][0]
    ...
    try:
        ...
        for one_session in session_all:
            license_list: dict = {}
            if base_id == one_session.getInfoBaseId().toString():
