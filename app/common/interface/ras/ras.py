...
не открыт порт на целевом сервере в firewall, нет маршрута до целевого сервера.")
        try:
            ...
            if req.debug is False:
                java_run['java_logger'].setLevel(jpype.JClass("java.util.logging.Level").OFF)
            else:
                java_run['java_logger'].setLevel(jpype.JClass("java.util.logging.Level").ALL)
            self.factory = java_run['AgentAdminConnectorFactory']()
