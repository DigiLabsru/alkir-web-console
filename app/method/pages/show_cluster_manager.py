...
from ..data.gen_table_method.get_cluster_manager import get_cluster_manager

show_admins_default_classes: str = 'nicegui-column w-[79%] h-full text-aqua bg-[#2A2A2A] rounded-lg p-2 flex-col'


def show_cluster_manager(connect_info, main_container):
...
async def load_data(container, connect_info):
    ...
    try:
        ...
        new_data = await run.io_bound(get_cluster_manager, connect_info=connect_info)
    ...
    with container.classes(f'{show_admins_default_classes} justify-start items-start'):
        ui.button(icon='autorenew', on_click=lambda: show_cluster_manager(main_container=container, connect_info=connect_info)).classes('ml-auto')
        ui.aggrid(
            options={
                "defaultColDef": {
                    "filter": True
